...
def detect_bezier_type(deg: int) -> str:
...
def draw_bezier():
    if dpg.does_alias_exist("bezier_theme"): dpg.remove_alias("bezier_theme")
    if dpg.does_alias_exist("annotation_theme"): dpg.remove_alias("annotation_theme")
    if dpg.does_alias_exist("curve_label_theme"): dpg.remove_alias("curve_label_theme")
    ...
    # Acquire data
    n_ctrl_points = dpg.get_value("ctrl_pts")
    smoothness = dpg.get_value("smooth")

    bezier_coords, ctrl_points = bz.bezier_curve_points(n_ctrl_points, smoothness)
    ...
    with dpg.plot(
        ...
        dpg.add_plot_annotation(
            label=detect_bezier_type(n_ctrl_points),
...
with dpg.window(tag="Primary Window"):
    ...
    dpg.add_input_int(label="Smoothness", width=100, tag="smooth", default_value=1000)
